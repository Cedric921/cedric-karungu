    ...
    <section id="portfolio" className="py-24 scroll-mt-28 bg-gray-50 dark:bg-[#100B17] transition-colors duration-300" ref={ref}>
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {PROJECTS.filter(project => activeFilter === 'All' || project.category === activeFilter).map((project, idx) => (
            <div key={idx} className={`group relative bg-white dark:bg-black rounded-2xl overflow-hidden border border-gray-200 dark:border-white/5 hover:border-accent-500/30 transition-all duration-300 shadow-md dark:shadow-none transform ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`} style={{ transitionDelay: isVisible ? `${idx * 75}ms` : '0ms' }}>
              ...
              <div className="p-8">
                <div className="flex justify-between items-start mb-4">
                  <div>
                    <span className="text-accent-600 dark:text-accent-400 text-xs font-semibold uppercase tracking-wider mb-2 block">{project.category}</span>
                    <h3 className="text-2xl font-bold text-gray-900 dark:text-white group-hover:text-accent-600 dark:group-hover:text-accent-400 transition-colors">{project.title}</h3>
                  </div>

                  <div className="flex items-center gap-3">
                    <a href={project.link} target="_blank" rel="noopener noreferrer" className="p-2 bg-gray-100 dark:bg-white/5 rounded-full hover:bg-accent-600 dark:hover:bg-accent-600 hover:text-white transition-all text-gray-500 dark:text-gray-400 cursor-pointer shadow-sm hover:shadow-md transform hover:-translate-y-0.5 transition-all duration-300" aria-label="Open project website"><Icons.ExternalLink /></a>
                    <a href={project.githubLink} target="_blank" rel="noopener noreferrer" className="p-2 bg-gray-100 dark:bg-white/5 rounded-full hover:bg-accent-600 dark:hover:bg-accent-600 hover:text-white transition-all text-gray-500 dark:text-gray-400 cursor-pointer shadow-sm hover:shadow-md transform hover:-translate-y-0.5 transition-all duration-300" aria-label="Open project GitHub"><Icons.Github /></a>
                  </div>
                </div>
                ...
                <div className="mt-4">
                  <a href={project.link} target="_blank" rel="noopener noreferrer" className="text-accent-600 dark:text-accent-400 hover:text-accent-700 dark:hover:text-accent-300 text-sm font-medium cursor-pointer transition-colors">more details →</a>
                </div>

                <div className="flex flex-wrap gap-2 mt-4">
                  {project.tags.map(tag => (
                    <span key={tag} className="px-3 py-1 bg-gray-100 dark:bg-white/5 rounded-full text-xs text-gray-600 dark:text-gray-300 border border-gray-200 dark:border-white/5">{tag}</span>
                  ))}
                </div>
              </div>
            </div>
        ...
        <div className={`text-center mt-12 transition-all duration-1000 delay-300 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
          <a href="https://github.com/Cedric921" target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 px-8 py-3 bg-accent-600 text-white rounded-full hover:bg-accent-700 transition-colors font-medium cursor-pointer shadow-md hover:shadow-lg transform hover:-translate-y-0.5 transition-all duration-300">See more projects<Icons.ExternalLink /></a>
        </div>
      </div>
    </section>
  );
